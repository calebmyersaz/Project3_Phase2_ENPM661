...
while True:
    ...
    if nodes[200-y1][x1][a1].free and nodes[200-y2][x2][a1].free:
...
while(iterations<20):
    ...
    if l1 == l2 and ((y2-y1)**2 + (x2-x1)**2) <= ((1.5*RobotRadius)**2):
    ...
    # If the current 'node' is not in the threshold region of 'goal' node, 'close' the node and explore neighbouring nodes
        ...
        for action in actions:
            ...
            # Returns Xn Yn Thetan and D(cost)
            phi = ActionMove[2]
            y = ActionMove[1]
            x = ActionMove[2]
            l = int(phi/30)
